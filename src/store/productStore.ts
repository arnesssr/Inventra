  ...
  devtools(
    persist(
      (set, get): ProductState => ({
        ...
        addProduct: async (product: CreateProductInput): Promise<Product> => {
        ...
        updateProduct: async (id: string, updates: Partial<Product>) => {
          ...
          try {
            if (updates.images && Array.isArray(updates.images)) {
              const imageWithPreviews: ImageWithPreview[] = updates.images.map(img => ({
                file: img,
                previewUrl: URL.createObjectURL(img)
              }));
              updates.imageUrls = get().handleImageUpdates(id, imageWithPreviews);
            }
        ...
        getRecentChanges: () => {
          return get().products
            .filter((product): product is Product & { updatedAt: string } => product.updatedAt !== undefined)
